...
| — PR Card Status Badges | 988 | Review/CI/divergence/approved-by-me badges, shared `BehindBadge`, Draft Toggle and Merge Actions (`DraftToggleButton`, `MergeButton` + confirm dialog, `--match-head-commit`) |
...
| **API Endpoints** | 2833 | All REST routes, grouped by domain (auth → cache); PR Actions (draft/merge) at 3124, Analytics Daily at 3252, Auto Verdicts at 3436, Automation/Reviewers (incl. `/api/automation/pipeline`) at 3543, Review Logs at 4312 |
| **Configuration** | 4430 | `config.json` options, incl. review retry + log retention, `log_retention_days`, `pr_sync` block (incl. `retain_days`, history walk + commit sync keys, `behind_per_cycle`), `analytics` block (`bot_logins`), `review_section_names` (two-tier keys); DB-backed settings keys note |
| **Technical Details** | 4520 | gh CLI integration, caching, parallel fetch, logging (UTC per-run files + error.log), attempt outcome + retry policy (4794), follow-up parent selection (4847), Review JSON Schema (4880: **2.0.0 two-tier `blocking`/`non_blocking` sections + schema history / `normalize_legacy_sections`**, `disputed`/`deferred` sections, per-issue `severity`/`disposition`) |
| **Future Considerations** | 5053 | Improvements, known limitations |
| **Appendix** | 5114 | Dependencies, file structure, run instructions |
